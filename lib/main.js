...
function initialize(settings, options) {
  ...
  if (Meteor.isClient && settings.client !== false) {
    ...
    client = window.Raven;
    client.config(settings.client, settings.config).install();
    isRavenEnabled = true;
  }
 ...
 * @param attrs object          Optionally add attributes such as 'extra', 'fingerprint', and 'level'
 */
function log(message, tags, attrs) {
  if (!isRavenEnabled) {
  ...
  var _tags = {tags: tags}
  var mergedAttrs = _.extend({}, attrs, _tags)
  if (message instanceof Error) {
    client.captureException(message, mergedAttrs);
  } else {
    client.captureMessage(message, mergedAttrs);
  }
}
